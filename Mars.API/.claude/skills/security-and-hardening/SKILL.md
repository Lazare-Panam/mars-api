...
**Reference:** Andrew Lock, *ASP.NET Core in Action* (3rd ed.) -- Ch. 23 (adding authentication with ASP.NET Core Identity), Ch. 24 (claims-based and resource-based authorization), Ch. 25 (securing Web APIs), Ch. 28 (HTTPS and certificates in production), Ch. 29 (protecting against XSS, CSRF, and configuring CORS). Point the human at the chapter matching the specific vulnerability class or auth concern at hand, not the whole book.
